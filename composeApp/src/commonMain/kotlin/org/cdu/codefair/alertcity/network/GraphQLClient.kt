...
class GraphQLClient {
    ...
    fun findAllUsers(): Flow<ApolloResponse<FindAllUsersQuery.Data>> = flow {
        emit(apolloClient.query(FindAllUsersQuery()).execute())
    }

    fun findEventsByDate(date: String) = flow {
        emit(apolloClient.query(FindEventsByDateQuery(date)).execute())
    }

    fun findEventsByOrgName(orgName: String) = flow {
        emit(apolloClient.query(FindEventsByOrgNameQuery(orgName)).execute())
    }

    fun findEventsBySubmitter(submitter: String) = flow {
        emit(apolloClient.query(FindEventsBySubmitterQuery(submitter)).execute())
    }

    fun findEventsByType(eventType: String) = flow {
        emit(apolloClient.query(FindEventsByTypeQuery(eventType)).execute())
    }
